...
const translate = (key: ObjectStringPath<typeof strings.de>): string => {
    // set the current language strings
    let languageStrings = strings[currentLanguage.value];
    ...
    for (let i = 0; i < keyParts.length; i++) {
        // @ts-expect-error TypeScript does not know the structure of languageStrings
        if (!languageStrings?.[keyParts[i]]) {
            console.warn(`Translation key not found: ${key}`);
            return key;
        }
        // @ts-expect-error TypeScript does not know the structure of languageStrings
        languageStrings = languageStrings[keyParts[i]];
    }
...
const translateCount = (key: ObjectStringPath<typeof strings.de>, count: number): string => {
    ...
    if (translationPluralizations.length === 2) {
        translation = count === 0 
            ? translationPluralizations[0]!.trim() 
            : translationPluralizations[1]!.trim()
        ;
